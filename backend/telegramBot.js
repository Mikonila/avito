...
async function sendWelcome(bot, msg) {
  ...
  await bot.sendMessage(
    ...
    `<b>${escapeTelegramHtml(firstName)}, добро пожаловать в Violet — бесплатный маркетплейс Черногории.</b>\n\n` +
      '<b>Здесь можно:</b>\n' +
      '• находить проверенных специалистов с хорошим рейтингом и отзывами\n' +
      ...
      '• находить вещи, которые люди отдают бесплатно\n' +
      '• просматривать рекомендованные объявления, подобранные под ваши интересы\n' +
      '• узнавать о предстоящих мастер-классах, лекциях, концертах, и объявлять о своих\n' +
      '• продвигать объявления дешевле чашки кофе и получать больше откликов\n\n' +
      '<b>Чтобы открыть маркетплейс, нажмите на кнопку ниже 👇</b>',
    {
      parse_mode: 'HTML',
